  ...
  async function createBrand(variableArr,brand,key){
    ...
    const h3 = '<div class="s1-content">\n<div class="s1-hdg-lv2"><h2 id="hdg-brand-'+key+'" class="s1-hdg-lv2__element">'+brand+'</h2></div>\n\n<div class="s1-content-area s1-content-first">'
    const tempHead = '<p class="scroll-txt">横にスクロールします</p>\n<div class="scroll-box">\n<div id="brand-'+key+'" class="s1-grid">'
    const tempFoot = '</div>\n</div>\n</div>\n</div>'
  ...
  async function createCalendar(variableArr,month) {
    ...
    let calendarHTML = '<div class="s1-grid__col-1-3"><table class="s1-tbl-data01 s1-tbl-data01--type01 ta-c"><thead><tr><th colspan="7">'+ monthNumber +'月</th></tr></thead><tbody><tr class="bg-g">';
    ...
    for (let i = 0; i < 6; i++) {
        ...
        for (let j = 0; j < 7; j++) {
            if (i === 0 && j < firstDay) {
                calendarHTML += `<td class="mute bg-g">&nbsp;</td>`;
                prevDayCount++;
              ...
              let dataKey = yearNumber + monthNumber00 + dayCount00
                if (j === 0 || j === 6) {
                    calendarHTML += `<td class="bg-r-02" data-day="`+ dataKey +`">${dayCount}</td>`;
                } else {
    ...
    calendarHTML = calendarHTML.replace(/<tr><td class="mute bg-g">&nbsp;<\/td><td class="mute bg-g">&nbsp;<\/td><td class="mute bg-g">&nbsp;<\/td><td class="mute bg-g">&nbsp;<\/td><td class="mute bg-g">&nbsp;<\/td><td class="mute bg-g">&nbsp;<\/td><td class="mute bg-g">&nbsp;<\/td><\/tr>/g, "");
    return calendarHTML;
  }
  ...
  async function matchHoliday(str,holidayArr){
    ...
    for (let i = 0; i < holidayArr.length; i++) {
      $str.find('[data-day="' + holidayArr[i] + '"]').addClass('bg-r-02');
    }
