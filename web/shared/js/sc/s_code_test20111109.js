/* SiteCatalyst code version: H.22.1 stg20111109 
...
s.dynamicAccountList="rakutenscrdev=accd.rakuten-sec.co.jp,ccwww.rakuten-sec.co.jp,devmw.rakuten-sec.co.jp,devmw2.rakuten-sec.co.jp,ifa.rakuten-sec.co.jp,member4.rakuten-sec.co.jp,member6.rakuten-sec.co.jp,nwcc.rakuten-sec.co.jp,stg.marketspeed.jp,stgmw.rakuten-sec.co.jp,stm01.rakuten-sec.co.jp,stgwww.rakuten-sec.co.jp;rakutenseccojpprod=rakuten-sec.co.jp,marketspeed.jp,marketspeed.dougahelp.com,kuji.rakuten.co.jp,ispeed.jp" 
s.currencyCode="JPY"
s.trackDownloadLinks=true
s.trackExternalLinks=true
s.trackInlineStats=true
s.linkDownloadFileTypes="exe,zip,wav,mp3,mov,mpg,avi,wmv,asx,pdf,doc,xls,ppt"
s.linkInternalFilters="javascript:,rakuten-sec.co.jp,marketspeed.jp,www.ris-asia.com,www.trkd-asia.com,knowledgebasket.net,marketspeed.dougahelp.com,id.rakuten.co.jp,kuji.rakuten.co.jp,ispeed.jp"
s.groupSiteFilters="rakuten.co.jp,rakuten.ne.jp,rakuten.com,rakuten.jp,infoseek.co.jp,rakuten-sec.co.jp,rakuten-kc.co.jp,ebank.co.jp,rakuteneagles.jp,nikki.ne.jp,shokutaku.jp,nuigurumi.ynot.co.jp,keibamall.co.jp,gol.com,onet.jp,onet.co.jp,marketspeed.jp,mytrip.co.jp,rakuten-bank.co.jp"
...
s.usePlugins=true
function s_doPlugins(s){try{
	//s.un swich for SmartPhone by UA
	var agent=navigator.userAgent
	if(agent.match(/iPhone|iPod|iPad|Android|IEMobile|NetFront/)){
		if(s.un.match(/dev/))s.un="rakutenscrsmrdev"
		else if(s.un.match(/prod/))s.un="rakutenscrsmrprod"
	}
	//1st party cookie vid
	...
	s.prop7=s.eVar7=s.getQueryParam("ric")
	...
	switch(s.pageName){
		...
		case "[member]bv:app:BRValid":
			...
			s.eVar5=s.pageName
			break
		case "[member]app:acc_friend_intro_confirm":
			s.events="event12"
			break
	}
	...
	if(s.getVisitStart()=="y"){
		...
		if(s.objCM){
			...
			if(s.eVar33=="Paid"){
				...
				s.eVar33+=":"+s.objCM.partner
				s.eVar34=s.eVar30=s.eVar31=s_rep(s.objCM.keyword,"+"," ")
				...
				//if(s.getCk("s_prevsite")==s.prop50)s.eVar33=s.eVar37=""
				if(s.campaign.match(/^R-/))s.eVar33="Group"
			}
			if(s.eVar33){
				s.eVar37="D=pageName"
	...
	s.eVar14=s.crossVisitParticipation(s.eVar33,"s_cpm","90","5",">","event2")
	...
	if(s.getCk("Rg_sec")){
		...
		//(2)-1
		s.prop1 = s.eVar1 = rgsec.substring(91,102)
		//(10)-9
		...
		//(2)-2
		var str2 =""
		for(var j = 349; j < 363; j++){
			if((j!=351)&&(j!=356)&&(j!=361)){
				var lt2 = rgsec.charAt(j)
				if(parseInt(lt2)==0){
				}else if(parseInt(lt2)<=4){
					lt2 = "1"
		...
		//(9)-2
		var str9 =""
		for(var n = 117; n < 120; n++){
			if(n!=119){
				var lt9 = rgsec.charAt(n)
				if(parseInt(lt9)==0){
				}else if(parseInt(lt9)<=4){
					lt9 = "1"
	...
	if(!s.eo&&!s.lnk&&!s.pageType&&!s.un.match(/dev/)&&!s.un.match(/rakutenglobal/)){
		if(s.eVar46||s.eVar49)s.un=s.apl(s.un,"rakutenglobalprod")
	}

	//s_code update
	s.prop37=s.apl(s.prop37,"stgmember111109")

}catch(e){}}s.doPlugins=s_doPlugins
/************************** PLUGINS SECTION *************************/
/*
...
+"_w(c,v,e?a:0);");
/*
 * Utility Function: read combined cookies v 0.3
 */
...
+"urn v;");}
/*
 * Utility Function: write combined cookies v 0.3
 */
...
s.visitorNamespace="rakuten";s.trackingServer="rakuten.112.2o7.net"
