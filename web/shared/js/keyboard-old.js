...
function openkey(){
    if(document.loginform !== null && document.loginform.loginid !== null && document.loginform.loginid !== undefined){
        if(focus_flag !== 0 && focus_flag !== 1){
            document.loginform.loginid.onfocus = function() { chfocus('0'); }
            document.loginform.loginid.focus();

        }
    } else if(document.LoginForm !== null && document.LoginForm.loginId !== null && document.LoginForm.loginId !== undefined){
        if(focus_flag !== 0 && focus_flag !== 1){
            document.loginform.loginid.onfocus = function() { chfocus('1'); }
            document.LoginForm.loginId.focus();
        }
    }
...
function chfocus(num){
    ...
    for(var j=0; j<in_nm_array.length; j++){
        ...
        if(focus_flag == j) {
            eval("document." + frmnm + "." + in_nm_array[j]).style.backgroundColor = "#FFF7D2";
        } else {
            eval("document." + frmnm + "." + in_nm_array[j]).removeAttribute('style');
        }
    }
    change_tab();
}
...
function change_tab(){
    if(focus_flag == 0){
        ...
        document.getElementById('key_tab0').innerHTML ="<img src=\"/web/shared/img/keyboard_tab_idon.gif\" alt=\"ログインID\" />";
        document.getElementById('key_tab1').innerHTML ="<img style=\"CURSOR: pointer\" onclick=chfocus(1) ondblclick=chfocus(1) src=\"/web/shared/img/keyboard_tab_passoff.gif\" alt=\"パスワード\" />";
        ...
        document.getElementById('key_tab0').innerHTML ="<img style=\"CURSOR: pointer\" onclick=chfocus(0) ondblclick=chfocus(0) src=\"/web/shared/img/keyboard_tab_idoff.gif\" alt=\"ログインID\" />";
        document.getElementById('key_tab1').innerHTML ="<img src=\"/web/shared/img/keyboard_tab_passon.gif\" alt=\"パスワード\" />";
