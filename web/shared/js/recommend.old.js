...
function readJson(JsonObj) {
	...
	for(var i=0;i<responseSize;i++){
		if( (json[i]['brandTypeCd'] == '1' && json[i]['brandCd'].slice(-1) == '0') 
		 || json[i]['brandTypeCd'] == '2' ){
			brandTypeCd[count] = json[i]['brandTypeCd'];
			brandCd[count] = json[i]['brandCd'];
			brandNm[count] = json[i]['brandNm'];
			marketCd[count] = json[i]['marketCd'];
			tseKbn[count] = json[i]['tseKbn'];
			nseKbn[count] = json[i]['nseKbn'];
			localMarketCd[count] = json[i]['localMarketCd'];
			count++;
		}
	}
...
function judgeDscrCd(){
	...
	if( dscrCd.length != 0 ){
		...
		/*本番用URL*/
		apiUrl = "https://www.ms2-rest-api.rakuten-sec.co.jp/api/recommend-stock/1?brandTypeCd="+ brandTypeCd + "&brandCd=" + dscrCd;
				...
				success: function(data, status) {
					...
					if(data != null){
						if(data["httpStatusCode"] == 200){
							...
							this.errorFunction(data["httpStatusCode"]);
							return false;
						} else {
							//
							this.errorFunction(data["httpStatusCode"]);
