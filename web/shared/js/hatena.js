...
B.apiOrigin = 'http://api.b.hatena.ne.jp';
B.staticOrigin = 'http://b.st-hatena.com';
if (/\.hatena\.ne\.jp:3000$/.test(location.host))
...
B.BookmarkButton = function (link) {
    this.url = B.BookmarkButton.extractURL(link.href) ||
...
extend(B.BookmarkButton.prototype, {
    setup: function () {
        var layout = this.link.getAttribute('data-hatena-bookmark-layout');
        if (layout === 'simple') {
            ...
            /* 15 分くらいならキャッシュされていても平気 */
            timestamp -= timestamp % (15 * 60);
            this.image = new Image();
            this.image.src = B.apiOrigin + '/entry/button/?url=' + encodeURIComponent(this.url) + '&layout=simple&format=image&_ts=' + timestamp;
        } else {
            var width = 50;
            var height = (layout === 'vertical') ? 50 : 20;
