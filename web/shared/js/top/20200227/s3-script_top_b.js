$(function() {
  ...
  // hero-carousel
      ...
      var $s3_hero = $('.s3-hero');
      var $s3_hero_carousel = $('.s3-hero-carousel');
      var slideNum = $(".s3-hero__container").length;
      $s3_hero_carousel.bxSlider({
        maxSlides: 2,
        auto: false,
        autoHover: true,
        touchEnabled: false,
        ...
        onSliderLoad: function(){
          $s3_hero.find('.bx-viewport > .s3-hero-carousel').css({
            width: slideNum * 100 + 415 + '%'
          });
          $s3_hero_carousel.animate({
            opacity: 1
          },500);
          $('.s3-hero .s3-hero__inner > .bx-wrapper .bx-pager, .s3-hero .s3-hero__inner > .bx-wrapper .bx-prev, .s3-hero .s3-hero__inner > .bx-wrapper .bx-next').addClass("fadeIn");
          $('.s3-hero .s3-hero__inner > .bx-wrapper .bx-prev, .s3-hero .s3-hero__inner > .bx-wrapper .bx-next').hover(
            function(){$s3_hero_carousel.stopAuto();},
            function(){$s3_hero_carousel.startAuto();}
          );
        }
      });
    });
  ...
  // member-carousel
  $(window).on('load', function(){
    var $s3_dashboard = $('.s3-member-banner');
    var $s3_dashboard_carousel = $('.s3-member-banner__list');
    var slideNum = $(".s3-member-banner__item").length;
    $s3_dashboard_carousel.bxSlider({
      maxSlides: 2,
      auto: true,
      autoHover: true,
      touchEnabled: false,
      ...
      speed: 500,
      onSliderLoad: function(){
        $s3_dashboard.find('.bx-viewport > .s3-member-banner__list').css({
          width: slideNum * 100 + 415 + '%'
        });
        $s3_dashboard_carousel.animate({
          opacity: 1
        },500);
        $s3_dashboard_carousel.children('li').find("img").trigger("slideappear");
      },
      onSlideBefore: function(){
        $s3_dashboard_carousel.children('li').find("img").trigger("slideappear");
      }
    });
    $('.bx-wrapper .bx-pager, .bx-wrapper .bx-prev, .bx-wrapper .bx-next').addClass("fadeIn");
  });
...
var tempFoot = '</tbody></table>'
  ...
  $.getJSON('/web/domestic/ipo/json/ipo_lead_json.json').then(
    function(data){
      ...
      var src = data.map(function(n,i){
        if(n.status!='クローズ' && n.status!=''){
          ...
          '<td>'+n.data18+'</td>'+
          '<td>'+n.code+'</td>'+
          '<td><a onclick="s.lidTrack(\'v_visitor_top_ipo_'+n.code+'\')" href="/web/domestic/ipo/'+n.code+n.sub+'.html">'+n.data1+'</a></td>'+
