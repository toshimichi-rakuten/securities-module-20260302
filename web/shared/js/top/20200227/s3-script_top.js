$(function() {
    ...
    window.callback = function(data) {
        var nikkeiAve = (data.market.list[0])[1],
            ...
            yenDollar = (data.market.list[32])[1],
            yenDollarPrev = (data.market.list[32])[2],
            yenDollarUpDown = (data.market.list[32])[3],
  ...
  fetch("/web/domestic/ipo/json/ipo_lead_json.json", {
    ...
    // 現在日時を取得 (時分秒を考慮せず、日付のみで比較するために00:00:00に設定)
    const now = getJapanTime();
    now.setHours(0, 0, 0, 0); // 今日の0時0分0秒に設定
    // console.log(now)
    ...
    var src = data.map(function (n, i) {
      // n.data18 が有効な日付文字列であることを確認
      if (n.data18 && n.data18 !== '-' && n.data18.match(/^\d{4}\/\d{2}\/\d{2}$/)) {
        // n.data18 を Date オブジェクトに変換
        const data18Date = new Date(n.data18);
        data18Date.setHours(0, 0, 0, 0);
        // 条件を修正: data18Date が現在日時 (now) より未来の場合
        if (data18Date > now) {
          return '<tr>' +
            '<td>' + n.data18 + '</td>' +
            '<td>' + n.code + '</td>' +
            '<td><a data-ratId="v_visitor_top_ipo_' + n.code + '" data-ratEvent="click" data-ratParam="all" href="/web/domestic/ipo/detail.html?code=' + n.code + '">' + n.data1 + '</a></td>' +
  ...
  fetch("/web/shared/json/info/info_red.json", {
	...
	cache: "no-store",
	  ...
	  for(let i in data){
		const mainFunc = function(srcx){ //メインの分岐条件を関数化
		  let flag = data[i].imp
		  return srcx + '<li>'+(flag =="〇" ? '<span class="s3-label-important">重要</span>' : '<span class=\"s3-label-'+ data[i].class +'\">'+ data[i].label +'</span>' )+'<a'+(data[i].blank =="〇" ? ' target=\"_blank\"' : '' )+' href="'+data[i].urpc+'" data-ratid="pc_top_visitor_info__'+data[i].slid+'" data-ratevent="click" data-ratparam="all">'+data[i].title+'</a></li>'
		}
		if(data[i].pcvtop != ""){
		  srcv = mainFunc(srcv);
		}
  ...
  var tempHead4 = '<ul class="s3-info-list">';
	var tempFoot4 = '</ul>'
  ...
  method: "get",
    ...
    for(let i in data){
      ...
      var tag = '<span class="s3-label-bule">'+tagArray[0]+'</span>'
      if(flagTime3 > 0){
        //if(cnt < 5){
          src = src + '<li><span>'+data[i].date+'</span>'+tag+'<a href="'+data[i].pcurl+'" data-ratid="v_visitor_top_seminar_'+data[i].rat+'" data-ratevent="click" data-ratparam="all">'+data[i].title+'</a></li>'
          cnt = cnt + 1;
        //}
      }
    }
  ...
  var tempHead5 = '<table class="s3-tbl-ipo">'+
  '<thead>'+
  '<tr>'+
  '<th>銘柄コード</th>'+
  '<th>対象銘柄</th>'+
  ...
  '</thead>'+
  '<tbody>';
  var tempFoot5 = '</tbody></table>'
  fetch("/web/domestic/tob/json/tob_lead_json.json", {
  method: "get",
  cache: "no-store",
    }).then(function (response) {
      return response.json();
      ...
      //data.sort(sort_by('data4', false, function(a){return a.toUpperCase()}));
      //data.sort(sort_by('data18', false, function(a){return a.toUpperCase()}));
      var src5 = data.map(function(n,i){
        return '<tr>'+
        '<td>'+n.code+'</td>'+
        '<td>'+n.data1+'</td>'+
        '<td>'+n.data3+'</td>'+
        '<td>'+n.data5+'～'+n.data7+'</td>'+
        '<td>'+n.data4+'円</td>'+
        '<td>'+n.data15+'</td>'+
        '</tr>';
      })
      .join('');
      if(src5!=''){
        src5 = tempHead5 + src5 + tempFoot5;
      }else{
        $("#TOBVTOP").remove();
      }
      src5 = src5.replace(/株式会社/g, '')
      src5 = src5.replace(/(\d{4})\/(\d{2})\/(\d{2})/g, '$2/$3')
      $('#data-json-tob-vtop').append(src5);
    },
    function(data){
      $("#TOBVTOP").remove();
    }
...
const $idPassForm = $('#idPassLoginForm');
const $passkeyForm = $('#passkeyLoginForm');
const $noPasskeyText = $('<p class="s3-form-login__no-passkey-text">パスキーを作成されていない方は</p>');

if (rg == null) {
  jQuery("#UADEACC").remove();
} else {
  // クッキー有の場合
  jQuery(".s3-form-login").addClass("s3-form-login--passkey");
  $passkeyForm.insertBefore($idPassForm);
  $noPasskeyText.insertBefore($idPassForm);
  jQuery("#FLOATACC").remove();
  // 会員トップからチャットボット削除
  jQuery("#ai-chat").remove();
  ...
  // PASSKEY
  // var flg = parseInt(rg.substr(804, 1));
  // if(flg == 1) {
  //   jQuery(".s3-form-login").addClass("s3-form-login--passkey");
  //   $passkeyForm.insertBefore($idPassForm);
  //   $noPasskeyText.insertBefore($idPassForm);
...
window.addEventListener('DOMContentLoaded', function(){
  ...
  setTimeout(function() {
    $('.vjack_wrap').addClass('slide');
  }, 1000);

var scrollStart = $('.s3-rc-h-standard').innerHeight();
var distance = 0;
...
$(document).scroll(function(){
  ...
  if (scrollStart <= distance) {
    $('.vjack_wrap').addClass('sticky');
  } else if (scrollStart >= distance) {
    $('.vjack_wrap').removeClass('sticky');
  }
});

$('.vjack_close').on('click', function () {
  $('.vjack_wrap').addClass('fade');
  setTimeout(function() {
    $('.vjack_wrap').remove();
  }, 300);
});
...
logbtn.onclick = () => {
    ...
    // idが 'idPassLoginForm' のフォームを取得して送信
    const idPassLoginForm = document.getElementById('idPassLoginForm');
    if (idPassLoginForm) { // 念のためフォームが存在するか確認
        idPassLoginForm.submit();
    } else {
        console.error("Error: Form with ID 'idPassLoginForm' not found.");
    }
};