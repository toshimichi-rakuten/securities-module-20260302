...
$(function () {
  ...
  fetch("/ITS/system_info_web.xml", {
    ...
    var utf8data = jconv.convert( data, 'EUCJP', 'UTF8' );
    console.log(utf8data);
    var parser = new DOMParser();
    var doc = parser.parseFromString(utf8data, "application/xml");
    $('#system_info').empty().append(doc.documentElement.firstChild.nodeValue.replace(/system_info\//g, '/ITS/system_info/'));
  });
  loadJSONP("https://www.trkd-asia.com/rakutensec/f_index1.jsp"); // IPOVTOP
  ...
  var tempHead = '<table class="s3-tbl-ipo">' + '<thead>' + '<tr>' + '<th>上場日</th>' + '<th>銘柄コード</th>' + '<th>銘柄名</th>' + '<th>ブックビルディング期間</th>' + '<th>仮条件</th>' + '<th>公開価格</th>' + '<th>購入申込期間</th>' + '<th>抽選日</th>' + '</tr>' + '</thead>' + '<tbody>';
  ...
  $(function () {
    fetch("/web/domestic/ipo/json/ipo_lead_json.json", {
      ...
      data.sort(sort_by('data4', false, function (a) {
        return a.toUpperCase();
      }));
      data.sort(sort_by('data18', false, function (a) {
        return a.toUpperCase();
      }));
      var src = data.map(function (n, i) {
        if (n.status != 'クローズ' && n.status != '') {
          return '<tr>' + '<td>' + n.data18 + '</td>' + '<td>' + n.code + '</td>' + '<td><a data-ratId="v_visitor_top_ipo_' + n.code + '" data-ratEvent="click" data-ratParam="all" href="/web/domestic/ipo/' + n.code + n.sub + '.html">' + n.data1 + '</a></td>' + '<td>' + n.data4 + '～' + n.data6 + '</td>' + '<td>' + n.data3 + '</td>' + '<td>' + n.data10 + '円</td>' + '<td>' + n.data11 + '～' + n.data13 + '</td>' + '<td>' + n.data17 + '</td>' + '</tr>';
        } else {
          return '';
        }
      }).join('');
      ...
      if (src != '') {
        ...
        src = '<p>現在、新規公開株式はございません。</p>'; //$('#IPOVTOP').addClass('is-hidden');
      }
  ...
  $(function () {
    fetch("/web/shared/json/info/info_red.json", {
      ...
      var _loop2 = function _loop2(_i) {
        detail = data[_i].title.match(regex);

        var mainFunc = function mainFunc(srcx) {
          //メインの分岐条件を関数化
          if (detail != null) {
            return srcx + '・' + detail[1] + '<a' + (data[_i].blank == "〇" ? ' target=\"_blank\"' : '') + ' href="' + data[_i].urpc + '" onclick="s.lidTrack(\'pc_top_important_' + data[_i].slid + '\')"><br>' + detail[2] + '</a>' + (_i != lastkey - 1 ? '<br>' : '');
          } else {
            return srcx + '<a' + (data[_i].blank == "〇" ? ' target=\"_blank\"' : '') + ' href="' + data[_i].urpc + '" onclick="s.lidTrack(\'pc_top_important_' + data[_i].slid + '\')">・' + data[_i].title + '</a>' + (_i != lastkey - 1 ? '<br>' : '');
          }
        };
...
jQuery.extend({
  csv: function csv(delim, quote, linedelim) {
...
function makeSettlementInfo() {
  ...
  if (jpflg == 0 && usflg == 0) {//jQuery("#SETTLEMENT_INFO").hide();
  }
}
...
makeSettlementInfo(); //NISA口座開設ボタンだし分け
